...
const SkewedRect = ({ piece }: { piece?: Piece }) => {
   ...
   */
  const sketch = ({ context, width, height }: SketchProps) => {
    const num = 30;
    const rects: Rect[] = [];
    ...
    for (let i = 0; i < num; i++) {
      ...
      const y = random.range(0, height * .5);
      const w = random.range(600, width * .5);
    ...
    return ({ context }: SketchProps) => {
      ...
      rects.forEach((rect) => {
        ...
        shadowColor = Color.offsetHSL(stroke, 0, 0, -20);
